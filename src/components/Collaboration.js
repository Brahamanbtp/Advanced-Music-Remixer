...
const Collaboration = () => {
  const { users, projectData, updateProject, isConnected } = useContext(CollaborationContext);
  ...
  const syncProjectWithServer = useCallback(async (changes) => {
    if (!isConnected) {
      console.warn('Collaboration.js: Socket is not connected. Cannot update project.');
      return;
    }
    setIsSyncing(true);
    try {
  ...
  useEffect(() => {
    const debouncedSync = debounce((changes) => syncProjectWithServer(changes), 500);
  ...
  return (
    <div className="collaboration">
      ...
      {!isConnected && <p className="error">⚠️ Not Connected to Server</p>}
      <div>
        <h4>Users</h4>
        <ul>
